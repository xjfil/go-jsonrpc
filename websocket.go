...
func (c *wsConn) sendRequest(req request) error {
	...
	defer c.writeLk.Unlock()
	if c.conn == nil {
		return fmt.Errorf("handler: websocket connection closed")
	}
...
func (c *wsConn) handleWsConn(ctx context.Context) {
	...
	for {
		...
		case r, ok := <-c.incoming:
			if !ok {
				if c.incomingErr != nil && !websocket.IsCloseError(c.incomingErr, websocket.CloseNormalClosure) && c.connFactory != nil {
					...
					go func() {
						...
						for conn == nil {
							select {
							case <-time.After(c.reconnectBackoff.next(attempts)):
							case <-ctx.Done():
		...
		case req := <-c.requests:
			if err := c.sendRequest(req.req); err != nil {
				...
				req.ready <- clientResponse{
					...
					ID:      *req.req.ID,
					Error: &respError{
						Message: fmt.Sprintf("send failure: %s", err),
						Code:    2,
					},
				}
			}
